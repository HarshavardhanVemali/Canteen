...
import json
from .models import Menu,Item,Cart,CustomUser,Prices,Order,AdminNotification,DeliveryPersonNotification,UserNotification,RestaurantNotification
...
class AllItemsConsumer(AsyncWebsocketConsumer): 
    ...
    @database_sync_to_async
    def get_item_with_cart(self, item_id):
        
        item = Item.objects.select_related('menu', 'submenu','resturant').get(id=item_id)
        cart_items = Cart.objects.filter(user=self.user, item_id=item_id) if not isinstance(self.user, AnonymousUser) else []
        cart_dict = {cart_item.item_id: cart_item.quantity for cart_item in cart_items}
        return item, cart_dict

    async def send_updated_item(self, item_id):
        try:
            item, cart_dict = await self.get_item_with_cart(item_id)
            item_data = {
                'item_id': item.id,
                'item_name': item.item_name,
            ...
            await self.send(text_data=json.dumps({'type': 'item_update', 'item': item_data}))
        except Exception as e:
            print(f"Error sending updated item: {e}")

    async def item_update(self, event):
        item_id = event.get('item_id')
        await self.send_updated_item(item_id)

    async def items_update(self, event):
         await self.send_current_items()

    @database_sync_to_async
    def get_items_with_cart_init(self):
        if self.user.is_anonymous:
            items = Item.objects.select_related('menu', 'submenu','resturant').order_by('-is_available')
        elif self.user.role == 'restaurant':
             items = Item.objects.select_related('menu', 'submenu','resturant').filter(resturant=self.user).order_by('-is_available')
        else:
            items = Item.objects.select_related('menu', 'submenu','resturant').order_by('-is_available')
    ...
    async def send_current_items(self):
        try:
            items, cart_dict = await self.get_items_with_cart_init()
            item_list = [
                {
                    'item_id': item.id,
                    'item_name': item.item_name,
            ...
            await self.send(text_data=json.dumps({'type': 'initial_items', 'data': item_list}))
        except Exception as e:
...
class SearchItemsConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        items, cart_dict = await self.get_filtered_items(search_term)
        item_list = [
            {
                'item_id': item.id,
                'item_name': item.item_name,
        ...
        await self.send(text_data=json.dumps({'type': 'search_result','items': item_list}))

    @database_sync_to_async
    def get_filtered_items(self, search_term):
        items = Item.objects.select_related('menu', 'submenu','resturant').filter(
            Q(item_name__icontains=search_term.strip()) | 
            Q(menu__menu_name__icontains=search_term.strip()) |  
            Q(submenu__sub_menu_name__icontains=search_term.strip()) | 
            Q(resturant__restaurant_name__icontains=search_term.strip()) 
        )
        cart_items = Cart.objects.filter(user=self.user) if not isinstance(self.user, AnonymousUser) else []
        cart_dict = {cart_item.item_id: cart_item.quantity for cart_item in cart_items}
...
class AdminNotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        user = self.scope.get('user')
        if user and user.is_authenticated and user.role == 'admin':
            await self.channel_layer.group_add('admin_notification_group', self.channel_name)
            await self.accept()
        else:
           await self.close()

    async def disconnect(self, code):
        await self.channel_layer.group_discard('admin_notification_group', self.channel_name)

    @database_sync_to_async
    def get_admin_notifications(self):
         return list(AdminNotification.objects.filter(delete_after__gt=timezone.now()).order_by('-created_at').values(
            'id', 'title', 'message', 'created_at', 'notification_type', 'related_order__order_id'))
    
    async def send_notification_update(self, event):
        notifications = await self.get_admin_notifications()
        notification_list = [
            {
                'id': notification['id'],
                ...
                'related_order': notification['related_order__order_id']
            }
            for notification in notifications
        ]
        await self.send(text_data=json.dumps({'type': 'admin_notification_update', 'notifications': notification_list}))

class DeliveryNotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        user = self.scope.get('user')
        if user and user.is_authenticated and user.role == 'delivery':
            self.delivery=CustomUser.objects.get(email=user.email)
            await self.channel_layer.group_add('delivery_notification_group', self.channel_name)
            await self.accept()
        else:
           await self.close()

    async def disconnect(self, code):
        await self.channel_layer.group_discard('delivery_notification_group', self.channel_name)

    @database_sync_to_async
    def get_delivery_notifications(self):
         return list(DeliveryPersonNotification.objects.filter(user=self.delivery,delete_after__gt=timezone.now()).order_by('-created_at').values(
            'id', 'title', 'message', 'created_at', 'notification_type', 'related_order__order_id'))
    
    async def send_notification_update(self, event):
        notifications = await self.get_delivery_notifications()
        notification_list = [
            {
                'id': notification['id'],
                'title': notification['title'],
        ...
        await self.send(text_data=json.dumps({'type': 'delivery_notification_update', 'notifications': notification_list}))
...
class RestaurantNotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if user and user.is_authenticated and user.role == 'restaurant':
            self.restaurant=CustomUser.objects.get(email=user.email)
            await self.channel_layer.group_add('restaurant_notification_group', self.channel_name)
            await self.accept()
        else:
           await self.close()

    async def disconnect(self, code):
        await self.channel_layer.group_discard('restaurant_notification_group', self.channel_name)

    @database_sync_to_async
    def get_restaurant_notifications(self):
         return list(RestaurantNotification.objects.filter(user=self.restaurant,delete_after__gt=timezone.now()).order_by('-created_at').values(
            'id', 'title', 'message', 'created_at', 'notification_type', 'related_order__order_id'))
    
    async def send_notification_update(self, event):
        notifications = await self.get_restaurant_notifications()
        notification_list = [
            {
                'id': notification['id'],
                'title': notification['title'],
        ...
        await self.send(text_data=json.dumps({'type': 'restaurant_notification_update', 'notifications': notification_list}))